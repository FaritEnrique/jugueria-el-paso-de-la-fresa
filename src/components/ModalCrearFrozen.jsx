...
const ModalProductos = () => {
    ...
    const { crearFrozen } = usePasoFresa()
    ...
    const handleSave = async (event) => {
        ...
        const respCrearFrozen = await crearFrozen(form)

        if (respCrearFrozen.id) {
            setForm({
                codigo: '',
                name: '',
                priceSmall: '',
                        ...
                        <div className='w-full py-4 flex justify-center'>
                            <div className='w-full bg-slate-400 py-4 px-6 rounded-xl'>
                                <div className='bg-white py-2'>
                                    <h2 className='mb-2 font-bold text-center'>Registre Nuevo Frozen</h2>
                                    <p className='px-4 text-justify'>Ingrese todos los campos para un registro satisfatorio</p>
                                </div>
                                <form onSubmit={handleSave} className='mt-4 p-2 w-full bg-black max-h-[440px] rounded-xl overflow-y-scroll'>
                                    <label className='flex flex-col gap-2 px-4 pt-4 text-white' htmlFor="">
                                        ...
                                        <input className='border border-black px-2 py-1 rounded-lg hover:border-blue-400 text-black' 
                                            type="text" 
                                            placeholder='Ejem. Para Frozen 2.1, 2.2 ...' 
                                            name='codigo'
                                            value={form.codigo}
                                            required
